...
class _SubscriptionManagementScreenState extends State<SubscriptionManagementScreen> {
  ...
  Future<void> _addOrUpdateSubscription(BuildContext context) async {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter a name for the subscription')),
      );
      return;
    }
    ...
    if (url.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter a URL for the subscription')),
      );
      return;
    }
    ...
    if (!_isUpdating) {
      final nameExists = provider.subscriptions.any((sub) => sub.name.toLowerCase() == name.toLowerCase());
      if (nameExists) {
        ...
        await showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text('A subscription with the name "$name" already exists. Please choose a different name.'),
            actions: [
              TextButton(
      ...
      final nameExists = provider.subscriptions.any(
        (sub) => sub.name.toLowerCase() == name.toLowerCase() && sub.id != _currentSubscriptionId
      );
      if (nameExists) {
        // Show error dialog for duplicate name
        await showDialog(
    ...
    try {
      if (_isUpdating && _currentSubscriptionId != null) {
        ...
        // Show loading indicator
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Adding subscription...')),
        );
  ...
  Future<void> _deleteSubscription(BuildContext context, Subscription subscription) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "${subscription.name}"? This will also remove all servers from this subscription.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancel', style: TextStyle(color: AppTheme.primaryGreen)),
          ),
          TextButton(
                ...
                child: Card(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          _isUpdating ? 'Edit Subscription' : 'Add Subscription',
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              onPressed: () => _addOrUpdateSubscription(context),
              ...
              Expanded(
                child: provider.isLoading
                    ...
                    : subscriptions.isEmpty
                        ? const Center(child: Text('No subscriptions added yet'))
                        : ListView.builder(
                                  ...
                                  trailing: Row(
                                    ...
                                    children: [
                                      IconButton(
                                        ...
                                        onPressed: () => _prepareForUpdate(subscription),
                                        tooltip: 'Edit',
                                      ),
                                      IconButton(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const Text('2. Enter the URL containing V2Ray configurations'),
              const Text('3. Click "Add" to save your subscription'),
              const Text('4. Use the refresh button to update configurations'),
            ],
          ),
        ),
        actions: [
